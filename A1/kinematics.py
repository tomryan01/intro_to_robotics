...
def euler(current_pose, t, dt):
  ...
  next_pose[X] += u*np.cos(current_pose[YAW])*dt
  next_pose[Y] += u*np.sin(current_pose[YAW])*dt
  next_pose[YAW] += w*dt
...
def rk4(current_pose, t, dt):
  ...
  # current_pose[YAW] is the current orientation of the robot.
  # Update next_pose[X], next_pose[Y], next_pose[YAW].
  f = lambda x, t: (euler(x, t, dt) - x)/dt
  k1 = f(current_pose, t)
  k2 = f(current_pose + dt*k1/2, t+dt/2)
  k3 = f(current_pose + dt*k2/2, t+dt/2)
  k4 = f(current_pose + dt*k3, t+dt)
  
  next_pose += 1/6 * (k1+2*k2+2*k3+k4)*dt
