...
def adaptiveIntegration(f, x0, t0, dt):
  xa = x0 + f(x0,t0, dt)*dt
  xhalf = x0 + f(x0,t0,dt)*dt/2
  xb = xhalf + f(xhalf, t0+dt/2,dt)*dt/2
  if np.linalg.norm(xa-xb) < 0.001:
    return xa
  else:
    return adaptiveIntegration(f, x0, t0, dt/2) + adaptiveIntegration(f, xhalf, t0+dt/2, dt/2) - xhalf
...
def euler(current_pose, t, dt):
  ...
  f = lambda x, t, dt: np.array([u * np.cos(x[YAW]), u * np.sin(x[YAW]), w(t)])
  return adaptiveIntegration(f, current_pose, t, dt)
...
def rk4(current_pose, t, dt):
  ...
  def rk4step(x0, t, dt):
    u = 0.25
    w = lambda t: np.cos(np.floor(t))
    f = lambda x, t: np.array([u * np.cos(x[YAW]), u * np.sin(x[YAW]), w(t)])
    k1 = f(x0, t)
    k2 = f(x0 + dt * k1 / 2, t + dt / 2)
    k3 = f(x0 + dt * k2 / 2, t + dt / 2)
    k4 = f(x0 + dt * k3, t + dt)

    return 1 / 6 * (k1 + 2 * k2 + 2 * k3 + k4)
  
  return adaptiveIntegration(rk4step, current_pose, t, dt)
