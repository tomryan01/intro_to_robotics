...
START_POSE = np.array([1., 0., 0.], dtype=np.float32)
MAX_ITERATIONS = 500
...
def sample_random_position(occupancy_grid):
  ...
  while not valid:
    ...
    valid = occupancy_grid.is_free(np.array([x,y]))
...
def adjust_pose(node, final_position, occupancy_grid):
  ...
  d = final_position - node.position
  alpha = np.arctan2(d[1], d[0])
  final_node = Node(np.append(final_position, 2.0*alpha - node.yaw))

  c, r = find_circle(node, final_node)
  theta_initial = np.arctan2((node.pose[:2]-c)[1], (node.pose[:2]-c)[0])
  theta_final = np.arctan2((final_position[:2]-c)[1], (final_position[:2]-c)[0])
  dt = occupancy_grid.resolution * np.sign(theta_final-theta_initial)
  N = int(np.ceil(np.abs(theta_initial-theta_final)/np.abs(dt)))
  theta = theta_initial
  for i in range(N):
    n = np.array([np.cos(theta), np.sin(theta)])
    if not occupancy_grid.is_free(r*n + c):
      return None
    theta += dt
  return final_node
